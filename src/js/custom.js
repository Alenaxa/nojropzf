...
// #NOTIFICATION_ 
	...
	$(document).click(function(event){
        ...
        // CHAT_MENU_OPEN
        // PAYBOX_OPEN 
        $('.PAYBOX_open').on('click', function() {
            $('.PAY_DETAIL_POPUPBOX').toggleClass('active');
            $('#pending-deposit').fadeOut(500);
        });
        $('.TRADE_PAYBOX_CLOSE').on('click', function() {
            $('.PAY_DETAIL_POPUPBOX').removeClass('active');
            $('#pending-deposit').fadeIn(500);
        });
        $(document).click(function(event) {
            if (!$(event.target).closest(".PAY_DETAIL_POPUPBOX, .PAYBOX_open").length) {
                $("body").find(".PAY_DETAIL_POPUPBOX").removeClass("active");
            }
        });
